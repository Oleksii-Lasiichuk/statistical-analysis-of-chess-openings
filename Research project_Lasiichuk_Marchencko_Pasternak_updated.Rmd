...
In this project, we analyzed around 20,000 online chess games to see if statistical methods could explain real-world player behavior. We tested two main ideas, and in both cases, we found significant results that allowed us to reject our null hypotheses.

### 1. Opening Choice Affects Game Length
First, we wanted to know if different openings lead to shorter or longer games. Since our data was not normally distributed (right-skewed), we chose the **Kruskal-Wallis test** instead of a standard ANOVA. 

* **Result:** We rejected the Null Hypothesis. There is a statistically significant difference between opening categories.
* **Specific Finding:** Post-hoc tests revealed that **Open Games ($1. e4\ e5$)** are uniquely faster and more volatile than other types. Meanwhile, Closed, Semi-Closed and Semi-Open games tend to cluster together as "slower" games. Essentially, playing an Open game creates a higher chance of a quick finish compared to playing a strategic system.

### 2. Rating Differences Predict Winners
Second, we tested if the rating gap accurately predicts who wins. We used **Logistic Regression** because the outcome is binary (Win/Loss).

* **Result:** We rejected the Null Hypothesis. The rating gap is a powerful predictor of the match result ($p < 2e^{-16}$).
* **Specific Finding:** The win probability follows a clear "S-curve." A bigger rating gap leads to a strictly higher chance of winning. However, we noticed that for extreme gaps (over 600 points), our empirical data diverged slightly from the model's predictions, likely due to the small sample size in that specific range.

### Final Thought
Overall, our analysis confirmed that chess isn't just random. By using methods like Kruskal-Wallis and Logistic Regression, we proved that both the opening you choose and the rating gap you have are statistically significant drivers of how a game will end.
